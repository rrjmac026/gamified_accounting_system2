...
@section('content')
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if (session('success'))
                <div class="mb-6 px-4 py-3 bg-green-100 border border-green-200 text-green-700 rounded-lg dark:bg-green-900/30 dark:border-green-700 dark:text-green-400">
                    {{ session('success') }}
                </div>
            @endif
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2">
                    <div class="bg-card rounded-xl shadow-md hover:shadow-lg transition-shadow duration-300 overflow-hidden dark:shadow-md">
                        <div class="border-b border-border-card p-6 bg-gradient-to-r from-card to-bg-card-inner">
                            <h2 class="text-xl font-semibold text-brand flex items-center text-text-primary">
                        ...
                        <div class="p-6 space-y-4">
                            @forelse($upcomingDeadlines as $task)
                                ...
                                class="block bg-bg-card-inner border border-border-card rounded-lg p-4 hover:bg-bg-hover transition-colors duration-200 dark:border-border-card">
                                    <div class="flex justify-between items-start">
                                        <div class="flex-1">
                                            <h3 class="font-medium text-text-primary mb-1">{{ $task->title }}</h3>
                                        ...
                                        <div class="text-right ml-4">
                                            ...
                                            @else
                                                <div class="text-sm text-text-muted">No due date</div>
                                            @endif
                                        </div>
                                    </div>
                                </a>
                            @empty
                                <div class="text-center py-8">
                                    ...
                                    <p class="text-text-secondary">No upcoming deadlines</p>
                                </div>
                            @endforelse
                        </div>
                    </div>
                    ...
                    <!-- Recent Grades -->
                    <div class="bg-card rounded-xl shadow-md hover:shadow-lg transition-shadow duration-300 overflow-hidden mt-6 dark:shadow-md">
                        <div class="border-b border-border-card p-6 bg-gradient-to-r from-card to-bg-card-inner">
                            <h2 class="text-xl font-semibold text-brand flex items-center text-text-primary">
                                <svg class="w-6 h-6 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/>
                        ...
                        <div class="p-6 space-y-4">
                            @forelse($recentGrades as $task)
                                <a href="{{ route('students.performance-tasks.show', $task->id) }}" 
                                   class="block bg-bg-card-inner border border-border-card rounded-lg p-4 hover:bg-bg-hover transition-colors duration-200">
                                    <div class="flex justify-between items-start">
                                        <div class="flex-1">
                                            <h3 class="font-medium text-text-primary mb-1">{{ $task->title }}</h3>
                                            <p class="text-sm text-text-secondary">
                                        ...
                                        <div class="text-right ml-4">
                                            @php
                                                ...
                                                $scoreColor = $scorePercentage >= 90 ? 'text-emerald-500 dark:text-emerald-400' : ($scorePercentage >= 70 ? 'text-amber-500 dark:text-amber-400' : 'text-red-500 dark:text-red-400');
                                            @endphp
                                            ...
                                            @if(isset($task->pivot->xp_earned) && $task->pivot->xp_earned)
                                                <div class="text-sm text-emerald-500 dark:text-emerald-400 font-medium mt-1">
                            ...
                            @empty
                                <div class="text-center py-8">
                                    ...
                                    <p class="text-text-secondary">No grades yet</p>
                                </div>
                            @endforelse
                        </div>
                    </div>
                    ...
                    <div class="bg-card rounded-xl shadow-md hover:shadow-lg transition-shadow duration-300 overflow-hidden dark:shadow-md">
                        ...
                        <div class="p-6">
                            <div class="text-center mb-6">
                                <div class="text-4xl font-bold text-brand mb-2">Level {{ $levelData['current_level'] }}</div>
                                <div class="text-sm text-text-secondary">{{ $levelData['xp_in_current_level'] }}/1000 XP to next level</div>
                            </div>
                            <div class="relative">
                                <div class="w-full bg-progress-track rounded-full h-4 overflow-hidden">
                                    <div class="bg-gradient-to-r from-brand to-brand-light h-4 rounded-full transition-all duration-500 shadow-sm" 
                                         style="width: {{ $levelData['progress_percentage'] }}%"></div>
                                </div>
                                <div class="text-center mt-2 text-xs text-text-muted font-medium">
                    ...
                    <div class="bg-card rounded-xl shadow-md hover:shadow-lg transition-shadow duration-300 dark:shadow-md">
                        ...
                        <div class="p-6 space-y-3">
                            @forelse($upcomingTasks as $task)
                                <a href="{{ route('students.performance-tasks.show', $task->id) }}" 
                                   class="block p-4 bg-bg-card-inner rounded-lg border border-border-card hover:bg-bg-hover transition-colors duration-200">
                                    <h3 class="font-medium text-text-primary">{{ $task->title }}</h3>
                                    <p class="text-sm text-text-secondary">{{ $task->subject->subject_name ?? 'No Subject' }}</p>
                                    @if($task->due_date)
                                        <div class="flex items-center mt-2 text-xs text-text-muted">
                            ...
                            @empty
                                <p class="text-text-muted text-center py-4">No upcoming tasks</p>
                            @endforelse
                        </div>
                    </div>
                </div>
