            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" id="taskList">
                @forelse ($performanceTasks as $task)
                    ...
                    <div class="task-card group bg-white rounded-xl shadow-sm hover:shadow-xl border border-gray-100 overflow-hidden transition-all duration-300 hover:-translate-y-1">
                        ...
                        {{-- Clickable body area --}}
                        <a href="{{ route('students.performance-tasks.show', $task->id) }}" class="block p-6 pb-3">
                            ...
                            {{-- Description --}}
                            <p class="task-desc text-sm text-gray-600 line-clamp-2 mb-4">
                        ...
                        </a>{{-- end clickable body --}}

                        {{-- ✅ Discussion button — outside the <a> so it doesn't trigger card navigation --}}
                        <div class="px-6 pb-5 pt-3 border-t border-gray-50">
                            @php
                                $unread = \App\Models\PerformanceTaskComment::where('performance_task_id', $task->id)
                                    ->where('sender_role', 'instructor')
                                    ->where('is_read', false)
                                    ->count();
                            @endphp

                            <a href="{{ route('students.performance-tasks.comments.show', $task->id) }}"
                               class="relative flex items-center justify-center gap-2 w-full px-4 py-2.5 rounded-xl border text-sm font-medium transition-all duration-200
                                      {{ $unread > 0
                                          ? 'bg-[#FF92C2] border-[#FF92C2] text-white shadow-sm hover:bg-[#ff79b5] hover:shadow-md'
                                          : 'bg-white border-gray-200 text-gray-600 hover:border-[#FF92C2] hover:text-[#FF92C2] hover:bg-[#FF92C2]/5' }}">
                                ...
                                @if($unread > 0)
                                    ...
                                    <span class="absolute -top-1.5 -right-1.5 inline-flex items-center justify-center min-w-[20px] h-5 px-1.5
                                                 bg-white text-[#FF92C2] border border-[#FF92C2] text-[10px] font-bold rounded-full shadow-sm">
                                        {{ $unread > 9 ? '9+' : $unread }}
                                    </span>
                                    <span class="absolute -top-1.5 -right-1.5 inline-flex min-w-[20px] h-5 rounded-full bg-[#FF92C2] opacity-40 animate-ping"></span>
                                @endif
                            </a>
                        </div>
